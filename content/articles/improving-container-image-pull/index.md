# Decreasing Container Image Pull Times
...
This was spawned because we noticed a long container start time that was negatively impacting the customer experience in processing work. If the customer expected something to process within 10 minutes, but we spent 5 minutes pulling the image and 3 minutes decompressing it, then we only had 2 minutes left to do the work.
...
p95 and p99 image pull times for an image this big was just under 5min as measured. This is what we were trying to reduce.

I had a lot of free reign to how to improve it, could add new components to the cluster, modify nodes, etc. However, I wanted to focus on something maintainable, something that worked with the node operating systems, and worked within the existing processes.
...
1. In AWS the base EKS node uses bottlerocket and in Azure the base AKS node uses AzureLinux. I'm not strictly limited to using these, but if I move off of these I need a really good reason because it will change things like node start-up time, security posture, and a lot more overall configuration.
1. This should minimum be cost neutral.
1. Floating tags are used. While @latest isn't used tags that correspond easily to versions are. This means that I need something that can handle looking at digests not tags.
...
1. We ship multiple times a day to non-prod and at least daily to most production environments. So whatever we do has to handle that kind of delivery speed.
...
The primary previous optimization we had already made in this area was: Regional endpoints and registries. This means that in AWS we use ECR endpoints local to the vpc the nodes are in and pull from an ECR registry that is local to the region. In Azure we use an ACR registry that lives in the same region as the AKS nodes. This, despite storing the same image duplicitively, was a big cost savings measure because of data transfer costs pulling from a centralized repo.

## Exploration of Options

There's not a ton of literature on optimizing this. It is only in the last few years that container images have both gotten this big AND images of this size matter for customer experiences. 

There were a few avenues and tools to explore:

### Pre-warming images

In some form, this means pulling the image before you need it. This could be at any number of moments:
- Pre-Node Boot
- Node boot
- Image build
- On Image push
...
1. [There's a nice thread on bottlerocket's github about using an EBS volume with the container images needed for a node](https://github.com/bottlerocket-os/bottlerocket/discussions/2023), also an [article from AWS](https://aws.amazon.com/blogs/containers/reduce-container-startup-time-on-amazon-eks-with-bottlerocket-data-volume/). However, this is too stale for us. The images that are large for us are models that change and cythonized python code. Both of which means that any EBS volume we snapshot has minimal shared layers with the ones pulled at container start.
2. [AWS has guidance on using SSM and eventbridge to fetching images on container push](https://aws.amazon.com/blogs/containers/start-pods-faster-by-prefetching-images/). While this is AWS specific, we could build something similar in azure land, or cross cloud, for example a daemonset that runs on every node, looks for image updates, and then executes image pulls. But this is also inefficient and means we need to adjust image pruning, how much disk we have on the node, etc. Possible, but not my first inclination.
...
Here there's less guidance. There are some cool tools I will talk about, like Dragonfly, Spegel, and Kraken, and some OS optimizations that can be done.
...
For the P2P tooling we have tools that fall into two camps: tools that modify the ContainerD config directly (Spegel and DragonFly) and tools that run node local container registries for optimized caching (Kraken). Here I ran into my first big issue. Bottlerocket doesn't allow for configuration of those ContainerD settings[[1]](https://github.com/spegel-org/spegel/issues/47)[[2]](https://github.com/bottlerocket-os/bottlerocket/issues/1963). So onto Kraken. Kraken works by deploying a node local contianer cache that then uses the other nodes as peers and the upstream registry to optimize container pulls.

However, here we run into our second problem. Spegel and Kraken both don't support floating tags very well. They both [recommend](https://spegel.dev/docs/guides/updating-latest-tag/) deploying something like Kyverno or k8s-digester to mutate pods from floating tags to digests. That's not a massive deal, but a little annoying. DragonFly gets around this by only operating at the digest layer. Rather than intercepting the whole image pull it intercepts only the layer requests.


#### DragonFly 

So, DragonFly was out first option. I wanted to see if I could get it to work on our Azure cluters to see what kind of performance gains we would see. Unfortunately, I was unable to get it to actually run. See, azure, like bottlerocket, doesn't expose all the containerD options. However, they have a slightly terrifying and novel solution of... just [modify containerD with a daemonset](https://learn.microsoft.com/en-us/azure/operator-nexus/howto-kubernetes-cluster-customize-workers). That was wild to discover. But that could work, so I gave it a shot.
...
I deployed out Kraken into one of our EKS testing clusters. This cluster has 5-6 complete running instances of the application at any given time and gets released to multiple times a day. This was a great cluster to test in. Getting it stood up wasn't terribly difficult, just had to provide credentials to our private registry. Getting Kyverno stood up also wasn't hard, I've run kyverno lots of times by now. The only slightly tricky part was writing the kyverno policy. We needed the policy to keep the deployment image the same, but modify both the digest and the registry.
...
Imagine my surprise when... image pull times got worse. Okay, so not statistically significantly worse. But just looking at the graph you can see that we had a lot more long pull times in both p95 and p99. 
...
See the kubelet metric doesn't break down the network calls from the decompression. So, that's my best guess. [EC2 Networking is symmetrical upload and download](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-network-bandwidth.html). So if we already had maxed out our download speed talking to ECR, and decompression time goes up because of the node local cache kraken introduces (nginx), then we would see a more erratic pattern and higher p99 and p95 times.
